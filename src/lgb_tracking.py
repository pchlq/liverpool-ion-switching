...
MODEL = 2  # os.path.basename(__file__).split('.')[0]
...
tt = pd.read_csv("../data/tt.csv")
# tt = pickle.load(open('../data/tt_10k.pkl', 'rb'))

# tt['signaleSqRo'] = np.sign(tt.signal) * np.abs(tt.signal)**0.5
print(tt.head(3))
...
sub[["time", "open_channels"]].to_csv(
    f"./sub_{MODEL}_{oof_f1:0.10f}.csv", index=False, float_format="%0.4f"
)
...
fi_df.sort_values("importance")["importance"].plot(
    kind="barh", figsize=(15, 30), title=f"{MODEL} - Feature Importance", ax=ax
)
