...
def run_lgb(train, test, y_train, params=None, cv=skf):
    ...
    for fold, (tr_ind, val_ind) in enumerate(cv.split(train, y_train)):
        ...
        model = lgb.train(
            params,
            lgb.Dataset(x_train, y_trainlgb),
            2000,
        ...
        oof_test[f"open_channels_fold_{fold}"] = model.predict(
            test, num_iteration=model.best_iteration
        )
        oof_df[f"open_channels_fold_{fold}"] = model.predict(
            train, num_iteration=model.best_iteration
        )
