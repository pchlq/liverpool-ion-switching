...
def read_data():
    train = pd.read_csv(
        DATA_DIR + "cleaned/train.csv",
        dtype={"time": np.float32, "signal": np.float32, "open_channels": np.int32},
    )
    test = pd.read_csv(
        DATA_DIR + "cleaned/test.csv", dtype={"time": np.float32, "signal": np.float32}
    )
...
def denois_train(df):
    ...
    ind_to_nan = train.iloc[nois_ind][
        (train.signal > upper) | (train.signal < low)
    ].index
    ...
    for i in range(len(tmp)):
        if tmp.loc[tmp.index[i], "open_channels"] != MAX_PROBA_COL[i]:
            ...
            # swapping probas
            (
                tmp.loc[tmp.index[i], f"proba_{true_col_idx}"],
                tmp.loc[tmp.index[i], f"proba_{pred_cod_idx}"],
            ) = (
                tmp.loc[tmp.index[i], f"proba_{pred_cod_idx}"],
                tmp.loc[tmp.index[i], f"proba_{true_col_idx}"],
            )
...
def batching(df, batch_size):
    df["group"] = (
        df.groupby(df.index // batch_size, sort=False)["signal"].agg(["ngroup"]).values
    )
...
def lag_with_pct_change(df, windows):
    for window in windows:
        df["signal_shift_pos_" + str(window)] = (
            df.groupby("group")["signal"].shift(window).fillna(0)
        )
        df["signal_shift_neg_" + str(window)] = (
            df.groupby("group")["signal"].shift(-1 * window).fillna(0)
        )
    return df
...
def split(GROUP_BATCH_SIZE=config.GROUP_BATCH_SIZE, SPLITS=config.SPLITS):
    ...
    train_tr = np.array(
        list(train.groupby("group").apply(lambda x: x[target_cols].values))
    ).astype(np.float32)
