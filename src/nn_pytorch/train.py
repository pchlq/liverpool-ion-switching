...
for index, (train_index, val_index, _) in enumerate(new_splits[0:], start=0):
    ...
    train_dataloader = DataLoader(
        train_dataset, config.NNBATCHSIZE, shuffle=True, num_workers=16
    )
    ...
    for epoch in range(config.EPOCHS):
        ...
        if res == 2:
            ...
            print(
                "folder %d global best val max f1 model score %0.5f"
                % (index, early_stopping.best_score)
            )
            break
        elif res == 1:
            print(
                "save folder %d global val max f1 model score %0.5f" % (index, f1_val)
            )

    # optimizer.update_swa()

    print(
        f"Folder {index} finally best global max f1 score is {early_stopping.best_score:0.5f}"
    )
    oof_score.append(round(early_stopping.best_score, 6))

    test_y = np.zeros(
        [int(2000_000 / config.GROUP_BATCH_SIZE), config.GROUP_BATCH_SIZE, 1]
    )
...
test_pred_frame = pd.DataFrame(
    {"time": ss["time"].astype(str), "open_channels": np.argmax(test_preds_all, axis=1)}
)
